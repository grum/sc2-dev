...
half4 PostProcessGaussianBlur( VertexTransport vertOut ) {
    ...
    for ( int i = 0; i < b_iSampleInterpolantCount; i++ ) {
        ...
        if ( b_iUseSeparateBlurMap )
            cValue = tex2D( p_sSeparateBlurMap, INTERPOLANT_GaussianBlurSample[i].xy );
        else cValue = tex2D( p_sSrcMap, INTERPOLANT_GaussianBlurSample[i].xy );
        half fWeight = p_fBlurWeights[i * 2 + 1];
        
        if ( b_iInteriorBlur ) {
            float4 vSampleNormalDepth = tex2D( p_sNormalDepthMap, INTERPOLANT_GaussianBlurSample[i].xy );
        ...
        else cValue = tex2D( p_sSrcMap, INTERPOLANT_GaussianBlurSample[i].zw );
        fWeight = p_fBlurWeights[i * 2 + 2];
        if ( b_iInteriorBlur ) {
            float4 vSampleNormalDepth = tex2D( p_sNormalDepthMap, INTERPOLANT_GaussianBlurSample[i].zw );
            if ( dot( vSampleNormalDepth.rgb, PIXEL_NORMAL < 0.9f ) || abs( vSampleNormalDepth.a - PIXEL_DEPTH ) > 0.01f )
                fWeight = 0.0f;
        }
        if ( b_iWeightedBlur )
            fWeight *= tex2D( p_sWeightMap, INTERPOLANT_GaussianBlurSample[i].zw ).a;
        if ( b_iLayeredBlur ) {
            float vDepth = tex2D( p_sNormalDepthMap, INTERPOLANT_GaussianBlurSample[i].zw ).a;
            if ( PIXEL_DEPTH < vDepth )    
                fWeight = 0.0f;
        }
        cResult += cValue * fWeight;
