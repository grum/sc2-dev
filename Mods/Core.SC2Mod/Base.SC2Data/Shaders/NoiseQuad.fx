...
sampler2D p_sNoiseMap;
sampler2D p_sHeightMap;
...
half4 NoiseQuadPixelMain( VertexTransport vertOut ) : COLOR {
    ...
    if (b_hackPass) {
        ...
        for (int x = -4; x <= 4; x++) {
            for (int y = -4; y <= 4; y++) {
                ...
                float4 cSample = tex2Dgrad(p_sHeightMap, uv, dx, dy);
    ...
    if (b_shadowGenerationPass) {
        ...
        float4 cSample0 = tex2D(p_sHeightMap, vertOut.vUV0.xy);
        float4 cSample1 = tex2D(p_sHeightMap, vertOut.vUV1.xy);
        float4 cSample2 = tex2D(p_sHeightMap, vertOut.vUV2.xy);
        ...
        float4 cNoiseSample0 = ((tex2D(p_sNoiseMap, noiseUV0.xy) - 0.5f) * 2.0f);
        float4 cNoiseSample1 = ((tex2D(p_sNoiseMap, noiseUV1.xy) - 0.5f) * 2.0f);
