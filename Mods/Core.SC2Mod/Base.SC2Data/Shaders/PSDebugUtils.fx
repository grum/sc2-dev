...
half4 DebugColor(   half3 cDiffuse, half3 cSpecular, half3 cEmissive, half3 vNormalMap, half3 vNormal, 
    ...
    if ( b_iDebugMode == TEXEL_DENSITY ) {
		half gradientX = length( ddx( vDiffuseUV * p_vDiffuseTextureSize ) );
		half gradientY = length( ddy( vDiffuseUV * p_vDiffuseTextureSize ) );
    ...
    if ( b_iDebugMode == UV_MAPPING )
        return half4( frac( vDiffuseUV.x ), frac( vDiffuseUV.y ), 0.0f, 1.0f );
