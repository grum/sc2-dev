...
float g_fParallaxShadow;

#if ( b_useParallaxMapping == 1 || CPP_SHADER )

//--------------------------------------------------------------------------------------------------
void LinearSearch (sampler2D sTex, int iChannels, int iNumSteps, inout float3 vVector, inout float3 vPoint) {
    vVector /= iNumSteps;
    for (int i=0; i<=iNumSteps; i++) {
        float h = ChooseChannel(iChannels, tex2D(sTex, vPoint.xy)).a;
        if (vPoint.z > h) {
            vPoint += vVector;
        }
    }
...
//--------------------------------------------------------------------------------------------------
void BinarySearch (sampler2D sTex, int iChannels, int iNumSteps, inout float3 vVector, inout float3 vPoint) {
    for( int i=0; i<iNumSteps; i++ ) {
        vVector *= 0.5f;
        float h = ChooseChannel(iChannels, tex2D(sTex, vPoint.xy)).a;
        if (vPoint.z > h) {
...
bool ComputeParallaxOffsetNew (
    ...
    LinearSearch(sHeightMap, iChannelSelect, 12, vViewVectorTS, vUV);
    BinarySearch(sHeightMap, iChannelSelect, 4, vViewVectorTS, vUV);

    vParallaxOffset = vUV.xy - vHeightMapUV;

    // shadow
    g_fParallaxShadow = 1;
    if (b_useParallaxMappingShadow && 0)
    {
        // this can be moved to vertex shader
        float3 vLightDirTS = mul(mTangent2World, p_directionalLights[0].vDirection.xyz);
        vLightDirTS /= abs(vLightDirTS.z);
        vLightDirTS.xy *= float2(p_vSpecularMultiplier_DepthBlendThreshold_HeightMapScale_AlphaFactor.z, -p_vSpecularMultiplier_DepthBlendThreshold_HeightMapScale_AlphaFactor.z);

        float3 vLightUV = vUV - vLightDirTS*(1-vUV.z);
        LinearSearch(sHeightMap, iChannelSelect, 5, vLightDirTS, vLightUV);
        if (vLightUV.z > vUV.z+0.01f)
            g_fParallaxShadow = 0;
    }
...
void OffsetUVEmitter(inout VertexTransport vertOut, int index, float4 offset)
{
    ...
    if(index % 2 == 0)
        INTERPOLANT_UV[index / 2].xy += offset.xy;
    else
        INTERPOLANT_UV[index / 2].zw += offset.xy;
#else
    INTERPOLANT_UV[index] += offset;
#endif
}

//--------------------------------------------------------------------------------------------------
void ApplyParallax( inout VertexTransport vertOut, sampler2D sHeightMap ) {
    float4 parallax = 0; 
    ...
    if (b_iUsePOMNew) {
		...
		// Height map and normal map must use the same mapping (they both share the same tangent basis). So height map uses the normal map mapping.
         if (ComputeParallaxOffsetNew( 
                ...
                GetUVEmitter(vertOut, b_iNormalUVEmitter), 
                sHeightMap, 
	    ...
	    // Height map and normal map must use the same mapping (they both share the same tangent basis). So height map uses the normal map mapping.
        parallax.xy = ComputeParallaxOffset( INTERPOLANT_ParallaxVector.xy, GetUVEmitter(vertOut, b_iNormalUVEmitter), sHeightMap, b_iHeightmapChannelSelect );
        for ( int i = 0; i < b_iUVEmitterCount; i++ ) {
            OffsetUVEmitter(vertOut, i, -parallax);         // Parallax is applied to all UVs.
        }
    }
...
void ApplyParallax( in VertexTransport vertOut, sampler2D sHeightMap ) {
}
