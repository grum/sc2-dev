...
//--------------------------------------------------------------------------------------------------
half SampleShadowMap( sampler2D shadowMap, float4 shadowMapUV ) {
	...
	if ( b_useShadows ) {
		...
		if ( b_iUseSoftShadows ) {
			...
			for ( int i = 0; i < b_iSoftShadowTaps / 2; i++ ) {
				...
				half4 rotOffset = rotation.xzxz * p_vSoftShadowTapOffsets[i].xxzz + rotation.yxyx * p_vSoftShadowTapOffsets[i].yyww;
				...
				cShadowColor += SampleShadowMap( p_sShadowMap, sampleUV + rotOffset.xyxx * half4( 1.0f, 1.0f, 0.0f, 0.0f ) );

				sampleUV = shadowMapUV;
                if ( b_iBiasAdjustment )
				    sampleUV.z += ddist_duv.x * rotOffset.z + ddist_duv.y * rotOffset.w;

				cShadowColor += SampleShadowMap( p_sShadowMap, sampleUV + rotOffset.zwxx * half4( 1.0f, 1.0f, 0.0f, 0.0f ) );
			}			
