...
float4 FlashPixelMain (in SFlashVtxFmt IN) : COLOR {
    ...
    if (b_iPixelShader == e_flashPixelShaderSolidColor) {
        ...
        outColor = tex2D(p_sTexture0, IN.vTC0.xy) * p_vColorTransformMul + p_vColorTransformAdd;
    }
    else if (b_iPixelShader == e_flashPixelShaderCxformTextureMultiply) {
        float4 color = tex2D(p_sTexture0, IN.vTC0.xy);
        ...
        color.a = color.a * tex2D(p_sTexture0, IN.vTC0.xy).a;
        outColor = color;
    }
        ...
        float4 color = lerp(IN.cColor0, tex2D(p_sTexture0, IN.vTC0.xy), IN.cFactor0.b);
        color = color * p_vColorTransformMul + p_vColorTransformAdd;
        color.a = color.a * IN.cFactor0.a;
        outColor = color;
    }
    else if (b_iPixelShader == e_flashPixelShaderCxform2Texture) {
        float4 color = lerp(tex2D(p_sTexture1, IN.vTC1.xy), tex2D(p_sTexture0, IN.vTC0.xy), IN.cFactor0.b);
